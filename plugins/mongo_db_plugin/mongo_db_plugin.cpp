...
void add_data(bsoncxx::builder::basic::document &act_doc, mongocxx::collection &accounts, const chain::action &act, const fc::microseconds &abi_serializer_max_time)
{
      ...
      {
            if (act.name != "onblock")
            { // eosio::onblock not in original eosio.system abi
...
{
      wlog("mongo_db_plugin_impl::filter");
      wlog("account: ${account}", ("account", act.account.to_string()));
      wlog("name: ${name}", ("name", act.name.to_string()));
      if (act.name.to_string() == "transfer")
      {
            wlog("name: ${name}", ("name", act.name.to_string()));
      }
      bool pass_on = false;
...
{
      cfg.add_options()("mongodb-queue-size,q", bpo::value<uint32_t>()->default_value(256),
                        "The target queue size between nodeos and MongoDB plugin thread.")("mongodb-wipe", bpo::bool_switch()->default_value(false),
                                                                                           "Required with --replay-blockchain, --hard-replay-blockchain, or --delete-all-blocks to wipe mongo db."
                                                                                           "This option required to prevent accidental wipe of mongo db.")("mongodb-block-start", bpo::value<uint32_t>()->default_value(0),
                                                                                                                                                           "If specified then only abi data pushed to mongodb until specified block is reached.")("mongodb-uri,m", bpo::value<std::string>(),
                                                                                                                                                                                                                                                  "MongoDB URI connection string, see: https://docs.mongodb.com/master/reference/connection-string/."
                                                                                                                                                                                                                                                  " If not specified then plugin is disabled. Default database 'EOS' is used if not specified in URI."
                                                                                                                                                                                                                                                  " Example: mongodb://127.0.0.1:27017/EOS")("mongodb-filter-on", bpo::value<vector<string>>()->composing(),
                                                                                                                                                                                                                                                                                             "Mongodb: Track actions which match receiver:action:actor. Actor may be blank to include all. Receiver and Action may not be blank.")("mongodb-filter-out", bpo::value<vector<string>>()->composing(),
            ...
            if (options.count("mongodb-uri"))
            {
                  ...
                  if (options.count("mongodb-filter-on"))
                  {
                        ...
                        for (auto &s : fo)
                        {
                              ...
                              wlog("mongodb-filter-on: receiver ${receiver}, action: ${action}", ("receiver", v[0])("action", v[1]));
